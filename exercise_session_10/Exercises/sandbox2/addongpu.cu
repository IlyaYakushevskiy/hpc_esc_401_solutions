...
int main () {
  ...
  cudaMemcpy(mat_in1, mat_in1_gpu, size, cudaMemcpyHostToDevice);
  cudaMemcpy(mat_in2, mat_in2_gpu, size, cudaMemcpyHostToDevice);
  cudaMemcpy(mat_out, mat_out_gpu, size, cudaMemcpyHostToDevice);
  ...
  kernadd<<<numBlocks,BLOCKSIZE>>>(mat_out_gpu, mat_in2, mat_in1 , nx, ny); 
  cudaDeviceSynchronize();
  
  /* We now transfer back the matrix from the device to the host */
  /* TO DO : write cudaMemcpy() instruction below */
  cudaMemcpy(mat_out_gpu, mat_out , size, cudaMemcpyDeviceToHost);
